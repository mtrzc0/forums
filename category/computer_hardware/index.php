        ...
        <div class="qa-container">
            <div class="qa-main">
                ...
                <div class="article-main-title-content">
                    <div class="article-main-title" style="margin-top: 10px">Computer hardware</div>
                </div>
                <main class="qa-main-content">
                    ...
                    try {
                        ...
                        if (!isset($_GET['sort']) || $_GET['sort'] == 'newest') {
                            $sql_query = 'SELECT p.id, p.title, p.category, p.content, p.votes, p.date, p.user_id, u.login, (SELECT COUNT(*) FROM Comments AS c WHERE c.post_id = p.id) AS comments FROM Posts AS p, Users AS u WHERE u.id = p.user_id AND p.category=\'Computer hardware\' ORDER BY p.date DESC';
                        } else if ($_GET['sort'] == 'oldest') {
                            $sql_query = 'SELECT p.id, p.title, p.category, p.content, p.votes, p.date, p.user_id, u.login, (SELECT COUNT(*) FROM Comments AS c WHERE c.post_id = p.id) AS comments FROM Posts AS p, Users AS u WHERE u.id = p.user_id AND p.category=\'Computer hardware\' ORDER BY p.date ASC';
                        } else if ($_GET['sort'] == 'hot') {
                            $sql_query = 'SELECT p.id, p.title, p.category, p.content, p.votes, p.date, p.user_id, u.login, (SELECT COUNT(*) FROM Comments AS c WHERE c.post_id = p.id) AS comments FROM Posts AS p, Users AS u WHERE u.id = p.user_id AND p.category=\'Computer hardware\' AND (DATEDIFF(NOW(), p.date) <= 7 AND p.votes > 100) ORDER BY p.date DESC';
                        } else if ($_GET['sort'] == 'top') {
                            $sql_query = 'SELECT p.id, p.title, p.category, p.content, p.votes, p.date, p.user_id, u.login, (SELECT COUNT(*) FROM Comments AS c WHERE c.post_id = p.id) AS comments FROM Posts AS p, Users AS u WHERE u.id = p.user_id AND p.category=\'Computer hardware\' AND (p.votes > 100) ORDER BY comments DESC';
                        } else {
                            $sql_query = 'SELECT p.id, p.title, p.category, p.content, p.votes, p.date, p.user_id, u.login, (SELECT COUNT(*) FROM Comments AS c WHERE c.post_id = p.id) AS comments FROM Posts AS p, Users AS u WHERE u.id = p.user_id AND p.category=\'Computer hardware\' ORDER BY p.date DESC';
                        }
